...
module {
  ...
  func write_data_to_buffer(x : Digest, data : () -> Nat8, sz : Nat, start : Nat) : (end : Nat) {
    if (start >= sz) return start;
    var i = start;
    while (x.i_byte < 8) {
      if (i == sz) return sz;
      Byte.writeByte(x, data());
      i += 1;
    };
    ...
    while (i < i_max) {
      ...
      Prim.nat32ToNat64(Prim.nat16ToNat32(Prim.nat8ToNat16(data()))) << 56
      ^ Prim.nat32ToNat64(Prim.nat16ToNat32(Prim.nat8ToNat16(data()))) << 48
      ^ Prim.nat32ToNat64(Prim.nat16ToNat32(Prim.nat8ToNat16(data()))) << 40
      ^ Prim.nat32ToNat64(Prim.nat16ToNat32(Prim.nat8ToNat16(data()))) << 32
      ^ Prim.nat32ToNat64(Prim.nat16ToNat32(Prim.nat8ToNat16(data()))) << 24
      ^ Prim.nat32ToNat64(Prim.nat16ToNat32(Prim.nat8ToNat16(data()))) << 16
      ^ Prim.nat32ToNat64(Prim.nat16ToNat32(Prim.nat8ToNat16(data()))) << 8
      ^ Prim.nat32ToNat64(Prim.nat16ToNat32(Prim.nat8ToNat16(data())));
      i += 8;
      i_msg +%= 1;
      if (i_msg == 16) {
