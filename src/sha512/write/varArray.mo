...
module {
  /// Internal SHA512 digest state used by the `[var Nat8]` writer.
  public type Digest = {
    // msg buffer
    msg : [var Nat64];
    var word : Nat64;
  ...
  /// Write the entire `[var Nat8]` array into the SHA512 message buffer, processing full blocks as they fill up.
  ...
  func write_data_to_buffer(x : Digest, data : [var Nat8], start : Nat) : (end : Nat) {
    let sz = data.size();
