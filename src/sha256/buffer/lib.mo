...
module {
  ...
  */
  ...
  /// Load bytes obtained via `at(i)` for `i` in `[start, sz)` into the SHA256 message buffer, stopping when the buffer fills (32 words) or when `i` reaches `sz`.
  /// Returns the index just past the last byte consumed.
