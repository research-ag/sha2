...
module {
  ...
  /// Run the SHA256 compression on exactly one 64-byte block read directly from
  /// two 32-byte blobs (`b1` -> message words 0..7, `b2` -> words 8..15),
  /// updating the 16 half-word state `self` in place. The caller must be at a
  /// block boundary (empty buffer) and supply two 32-byte blobs; no message
  /// buffer, no closures, no allocation.
