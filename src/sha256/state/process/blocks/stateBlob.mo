...
module {
  ...
  /// are the 32-byte digest `s1` and words 8..15 are the 32-byte blob `b2`,
  /// starting the compression from the SHA256 IV, and overwrite `self` with the
  /// result. `s1` is read into locals first, so it may alias `self`. Produces
  /// the first compression of SHA256(s1 ++ b2); the caller adds the padding
  /// block. No message buffer, no allocation.
  public func process(self : [var Nat16], s1 : [var Nat16], b2 : Blob) : () {
    // message words 0..7 from s1, 8..15 from b2
