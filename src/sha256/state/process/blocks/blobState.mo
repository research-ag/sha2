...
module {
  ...
  /// Inner block of a mixed combine: hash one 64-byte block whose words 0..7
  /// are the 32-byte blob `b1` and words 8..15 are the 32-byte digest `s2`,
  /// starting the compression from the SHA256 IV, and overwrite `self` with the
  /// result. `s2` is read into locals first, so it may alias `self`. Produces
  /// the first compression of SHA256(b1 ++ s2); the caller adds the padding
  /// block. No message buffer, no allocation.
  public func process(self : [var Nat16], b1 : Blob, s2 : [var Nat16]) : () {
    // message words 0..7 from b1, 8..15 from s2
