...
| [`MerkleCounter.mo`](./MerkleCounter.mo)     | The same Merkle root via a BINARY-COUNTER mountain range instead of a peak stack: a height-indexed `[var Hasher]` where the occupied slots are the bits of the leaf count (no `level[]` array), adding a leaf is a binary increment with carry. Height 0 is a pending `Blob` fused with `combineBlob32` (no per-leaf deserialize) and risen peaks move into place with O(1) reference SWAPs — so it matches/slightly beats the peak stack. Power-of-two leaf counts. |
