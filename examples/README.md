...
`combineStateBlob32`, `loadBlob32`, `loadState`, `readSum`) and the buffered
`Digest` (`close`, `closeDouble`, `readSum`, `sumDouble`).
...
The only allocations made by the code in the examples are the creation of the engines — the `Hasher`s, plus one reused `Digest` in the message-fed examples — and the final root hash Blob.

The seven Merkle examples form a progression — each adds one concept:
...
| 2   | [`MerkleStackState.mo`](./MerkleStackState.mo)     | The same as Example 1 but fed real MESSAGES instead of ready 32-byte leaf Blobs: the MMR embeds one reused `Digest` that absorbs each (arbitrary-length) message, and the leaf — SHA256(msg) — is read straight from the digest's state, no intermediate `Blob`. The unpaired leaf can no longer wait out of band as a pending `?Blob` (it exists only in the digest's state, which the next message overwrites), so it is captured into a stack slot as a level-0 peak (no pending field and no counter — a level-0 stack top IS the parked leaf), making the pool `ceil(log2 n) + 1`, exactly one `Hasher` more than Example 1 for every `n`: the pool size is decided by where the leaf lives, not by the organization. Bagging needs no pending-leaf special case: the parked leaf is simply the lowest component, on top of the stack. Also usable incrementally.                                                                                            |
...
| 4   | [`MerkleCounterState.mo`](./MerkleCounterState.mo) | The same as Example 3 but fed real messages through an embedded `Digest`, as in Example 2 — the counter organization and the digest bridge combined. The pool is `ceil(log2 n) + 1`, identical to Example 2, confirming that where the leaf lives alone decides the pool size. Example 6 is the double-SHA (Bitcoin) version of this example; there even the parking copy disappears, because the second SHA transports the leaf into its slot. Also usable incrementally — the bagging accumulator is the top slot of the array, one level above the highest possible peak (slot 0 holds the parked leaf and cannot serve).                                                                                                                                                                                                                                                                                                                                      |
...
| 7   | [`BitcoinWitness.mo`](./BitcoinWitness.mo)         | The COINBASE INCLUSION PROOF — the "merkle branch" of SPV proofs and mining protocols — for the Bitcoin trees of Examples 5 and 6. The capture lives in the shared bases, behind the `witness` flag of `new`: two O(1) hooks in the carry copy each left-spine sibling into a pre-sized `[var Blob]` (slot k = level k, matching the pre-allocated hasher pool) in the one carry step it exists — `ceil(log2 n)` small Blob allocations across the whole build, no extra `Hasher`s; the Blobs ARE the proof output, final on capture and shared into every proof. This file holds the extraction — `coinbaseWitness` (Example 5 trees) and `coinbaseWitnessTx` (Example 6 trees) return the spine plus the right remainder, computed per proof by stopping the collapse one step early — and the receiver side: `verifyTxid`/`verifyTx` check a supplied coinbase (txid, or the raw transaction) plus witness against the single 32-byte root the receiver holds. |
...
[`CounterBase.mo`](./CounterBase.mo), and Examples 4 and 6 share theirs
(including the embedded `Digest`) via
[`CounterStateBase.mo`](./CounterStateBase.mo); within each pair the examples
differ only in single vs double SHA256 — which for the digest-fed pair also
decides how a leaf exits the digest: a `loadState` capture vs the free
`hashState` bridge — and in the finalization code — bagging vs the Bitcoin
collapse — which lives in the example files. Both bases can optionally
capture the coinbase witness spine as leaves are added (the `witness` flag
of `new`), consumed by Example 7.
...
- Witness capture (`new(_, true)` on Examples 5 and 6, extraction in Example 7): no additional `Hasher`s — only a pre-sized `[var Blob]` of `ceil(log2 n)` slots holding the coinbase's left-spine siblings, written at their capture moments.
...
where the counter uses one `Hasher` more. The digest-fed sizes agree with
...
leaf that exists only as the digest's state — about to be overwritten by the
next message — must be captured into a slot the moment it arrives. The extra
...
Examples 1–4 build the generic single-SHA mountain-range root — the
message-fed Examples 2 and 4 hash each message first, so their root over the
messages equals Examples 1/3 over the hashed leaves; Examples 5–6 build
Bitcoin's double-SHA tree with its last-node duplication. Neither is RFC 6962, which prepends a
